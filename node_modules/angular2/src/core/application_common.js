...
function _injectorBindings(appComponentType) {
    ...
    return [
        ...
        di_1.bind(exception_handler_1.ExceptionHandler).toFactory(function () { return new exception_handler_1.ExceptionHandler(dom_adapter_1.DOM, lang_1.isDart ? false : true); }, []),
...
function createNgZone(handler) {
    ...
    var bootstrapErrorReporter = function (exception, stackTrace) { return handler.call(exception, stackTrace); };
...
function commonBootstrap(appComponentType, componentInjectableBindings) {
    ...
    var zone = createNgZone(new exception_handler_1.ExceptionHandler(dom_adapter_1.DOM, lang_1.isDart ? false : true));
    zone.run(function () {
        // TODO(rado): prepopulate template cache, so applications with only
        // index.html and main.js are possible.
        var appInjector = _createAppInjector(appComponentType, componentInjectableBindings, zone);
        var exceptionHandler = appInjector.get(exception_handler_1.ExceptionHandler);
        zone.overrideOnErrorHandler(function (e, s) { return exceptionHandler.call(e, s); });
        try {
            var compRefToken = appInjector.get(application_tokens_1.appComponentRefPromiseToken);
