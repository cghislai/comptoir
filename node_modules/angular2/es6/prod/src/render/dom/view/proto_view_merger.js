...
import { RenderProtoViewMergeMapping, ViewType, ViewEncapsulation } from '../../api';
import { NG_BINDING_CLASS, NG_CONTENT_ELEMENT_NAME, cloneAndQueryProtoView, queryBoundElements, queryBoundTextNodeIndices, NG_SHADOW_ROOT_ELEMENT_NAME } from '../util';
export function mergeProtoViewsRecursively(protoViewRefs) {
    // clone
...
function indexBoundTextNodes(mergableProtoViews) {
    ...
    for (var pvIndex = 0; pvIndex < mergableProtoViews.length; pvIndex++) {
        ...
        mergableProtoView.boundTextNodes.forEach((textNode) => { boundTextNodeMap.set(textNode, null); });
    }
...
function extractFragmentNodesFromElements(fragmentElements) {
    return fragmentElements.map((fragmentElement) => { return DOM.childNodesAsList(DOM.content(fragmentElement)); });
}
...
function createRootElementFromFragments(fragments) {
    ...
    var rootNode = DOM.content(rootElement);
    fragments.forEach((fragment) => { fragment.forEach((node) => { DOM.appendChild(rootNode, node); }); });
    return rootElement;
}
