...
import { Renderer, RenderCompiler, DirectiveMetadata, ProtoViewDto, ViewDefinition, RenderProtoViewRef, RenderProtoViewMergeMapping, RenderViewRef, RenderFragmentRef } from "angular2/src/render/api";
...
export let WebWorkerMain = class {
    constructor(_renderCompiler, _renderer, _renderViewWithFragmentsStore, _serializer, rootUrl) {
    ...
    _wrapWorkerPromise(id, promise, type) {
        PromiseWrapper.then(promise, (result) => {
            try {
                this._sendWorkerMessage("result", { "id": id, "value": this._serializer.serialize(result, type) });
            }
            catch (e) {
                print(e);
            }
        }, (error) => { this._sendWorkerError(id, error); });
    }
};
WebWorkerMain = __decorate([
    Injectable(), 
    __metadata('design:paramtypes', [RenderCompiler, Renderer, RenderViewWithFragmentsStore, Serializer, AnchorBasedAppRootUrl])
], WebWorkerMain);
