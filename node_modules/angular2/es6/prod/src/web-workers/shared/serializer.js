...
import { ProtoViewDto, DirectiveMetadata, ElementBinder, DirectiveBinder, ElementPropertyBinding, EventBinding, ViewDefinition } from "angular2/src/render/api";
import { ASTWithSource } from "angular2/change_detection";
export class Serializer {
    static serialize(obj, type) {
        if (!isPresent(obj)) {
            return null;
        }
        if (isArray(obj)) {
    ...
    static deserialize(map, type, data) {
        if (!isPresent(map)) {
            return null;
        }
        if (isArray(map)) {
     ...
     */
    static objectToMap(obj, type, data) {
        if (isPresent(type)) {
            var map = new Map();
            StringMapWrapper.forEach(obj, (key, val) => { map.set(key, Serializer.deserialize(val, type, data)); });
            return map;
        }
        else {
...
class DirectiveBinderSerializer {
    static serialize(binder) {
        return {
            ...
            'hostPropertyBindings': Serializer.serialize(binder.hostPropertyBindings, ElementPropertyBinding)
        };
    }
    static deserialize(obj) {
